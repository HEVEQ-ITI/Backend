...
{
    public sealed class AcceptBookingCommandHandler : IRequestHandler<AcceptBookingCommand, BookingDto>
    {
        ...
        public async Task<BookingDto> Handle(AcceptBookingCommand request, CancellationToken cancellationToken)
        {
            ...
            var operatorExits = await _context.Operators.AnyAsync(
            ...
            x.ListingId == booking.ServiceListingId &&
            x.OperatorId == request.OperatorId &&
            x.Date == booking.RequestedStartDate,
            cancellationToken);
